...
class ToDo(ToDoTemplate):
  ...
  def load_new_todo(self):
    ...
    self.new_todo_gp.add_component(DropDown(width=200), tag='new_todo_group_name')
    self.new_todo_gp.add_component(Button(text='Cancel', tag='new_todo_cancel_btn'))
    self.new_todo_gp.add_component(Button(text='OK', tag='new_todo_ok_btn'))
