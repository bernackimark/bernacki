...
class ToDo(ToDoTemplate):
  def __init__(self, **properties):
    ...
    new_todo_tb = TextBox(width=200, tag='new_todo_name')
    new_todo_existing_group_lbl = Label(text='Assign to To-do Group:')
    new_todo_existing_group_dd = DropDown(width=200, tag='new_todo_group_name')
    new_todo_cancel_btn = Button(text='Cancel', tag='new_todo_cancel_btn')
    new_todo_ok_btn = Button(text='OK', tag='new_todo_ok_btn')
  ...
  def load_new_todo(self, new_todo_lbl, new_todo_tb, new_todo_existing_group_lbl, new_todo_existing_group_dd, new_todo_cancel_btn, new_todo_ok_btn):
    ...
    self.new_todo_gp.add_component(new_todo_ok_btn, row=2, col_xs=7, width_xs=1)
